...
fp8_gemm_kernel_swapAB(__nv_bfloat16* gmem_d, float* scales_a, typename SchedulerType::Input problem_input,
    ...
    if (threadIdx.x >= kNumMathThreads) {
        ...
        while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
            ...
            launch_k_iterations([&](int k_iter, auto type) {
                ...
                for (int s = 0; s < kNumInnerStages; ++ s) {
                    ...
                    for (int i = 0; i < WGMMA::kNumAccum / 4; ++ i) {
                        ...
                        final_accum[i * 4 + 2] += scale_0_0 * accum[i * 4 + 2];
                        final_accum[i * 4 + 3] += scale_0_1 * accum[i * 4 + 3];
                    }
                }
            ...
            if constexpr (SchedulerType::gemm_type == GemmType::GroupedWithOffset) {
                // if(scheduler.curr_group_idx == 0 && m_block_idx == 0 && n_block_idx == 0 && threadIdx.x == 0) {
                //     printf("m_block_idx: %d, n_block_idx: %d, n_global_idx: %d, n_boundary: %d\n", m_block_idx, n_block_idx, scheduler.get_global_n_idx(n_block_idx), scheduler.n_boundary);
                //     printf("smem_d: %p\n", smem_d);
                //     for(int i = 0; i < 4; i++) {
                //         for(int j = 0; j < 3; j++) {
                //             printf("(%d %d): %f ", i, j, float(smem_d[i * BLOCK_N + j]));
                //         }
                //         printf("\n");
                //     }
                // }
    ...
    //     for(int i = 0; i < 4; i++) {
    //         for(int j = 0; j < 3; j++) {
    //             printf("(%d %d): %f ", i, j, float(gmem_d[i * SHAPE_M  + j]));
    //         }
    //         printf("\n");
    //     }
    // }
