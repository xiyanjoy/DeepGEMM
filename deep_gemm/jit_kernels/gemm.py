...
// Launch kernel
auto tma_a_desc = GemmType::make_2d_tma_a_desc(lhs, m, ld_lhs_in_bytes);
...
GemmType::run(out, rhs_scales, static_cast<int*>(nullptr),
              m,
...
// Make a templated GEMM
using GemmType = GemmSwapAB<M, K, BLOCK_M, BLOCK_N, 128, 1, kNumStages, kNumTMAMulticast, GemmType::Normal>;

// Launch kernel
auto tma_a_desc = GemmType::make_2d_tma_a_desc(lhs, ld_lhs_in_bytes);  // a是左值,weight
auto tma_b_desc = GemmType::make_2d_tma_b_desc(rhs, n, ld_rhs_in_bytes);  // b是右值,act
auto tma_scales_b_desc = GemmType::make_2d_tma_scales_b_desc(rhs_scales, n);  // act scales
auto tma_d_desc = GemmType::make_2d_tma_d_desc(out, n, ld_out_in_bytes);
GemmType::run(out, lhs_scales, static_cast<int*>(nullptr),
              n,
...
def get_smem_size(num_stages: int, k: int, block_m: int, block_n: int, block_k: int = 128, swapAB: bool = False) -> int:
    if not swapAB:
...
def get_best_configs(m: int, n: int, k: int, num_groups: int, num_sms: int,
                     is_grouped_contiguous: bool = False, swapAB: bool = False) -> Tuple[int, int, int, int, int]:
    ...
    # print(f'best_block_m: {best_block_m}, best_block_n: {best_block_n}, best_num_waves: {get_num_waves(best_block_m, best_block_n)}, best_util: {get_last_wave_util(best_block_m, best_block_n)}, num_sms: {num_sms}')
    ...
    for num_stages in (6, 5, 4) if 128 % best_block_n != 0 else (8, 7, 6, 5, 4):
        best_smem_size = get_smem_size(num_stages, k, best_block_m, best_block_n, swapAB=swapAB)
...
def gemm_fp8_fp8_bf16_nt(lhs: Tuple[torch.Tensor, torch.Tensor],
    ...
    args = (lhs, ld_lhs * 1, lhs_scales, rhs, ld_rhs * 1, rhs_scales, out, ld_out * 2, m, torch.cuda.current_stream(), num_sms, smem_size)
    runtime = jit_tuner.compile_and_tune(
        ...
        space=(),
        includes=includes,
        arg_defs=(('lhs', torch.float8_e4m3fn), ('ld_lhs_in_bytes', int), ('lhs_scales', torch.float),
                  ('rhs', torch.float8_e4m3fn), ('ld_rhs_in_bytes', int), ('rhs_scales', torch.float),
                  ('out', torch.bfloat16), ('ld_out_in_bytes', int), ('m', int),
...
def gemm_fp8_fp8_bf16_nt_swapAB(lhs: Tuple[torch.Tensor, torch.Tensor],
                             ...
                             ld_out: int = 0) -> None:
    """
    Do a normal GEMM with FP8 inputs and BF16 output, with swapped A and B matrices.
    LHS (weight), LHS scaling factors, RHS (act), and output tensors must be in contiguous format.
    The RHS scaling tensor requires TMA-aligned transposed format, if your input does not match the requirement,
        this function will do a transposing with a set of slow PyTorch operations.

    Arguments:
        lhs: the first element is an FP8 tensor (typed `torch.float8_e4m3fn`) of shape `[m, k]` (weight),
             the second element is an FP32 128x128 scaling tensor for LHS of shape `[⌈m / 128⌉, ⌈k / 128⌉]`.
        rhs: the first element is an FP8 tensor (typed `torch.float8_e4m3fn`) of shape `[n, k]` (act).
    ...
    m, k = lhs.shape
    n, k_ = rhs.shape
    n_, m_ = out.shape  # 输出是[n, m]而不是[m, n]

    assert m % 64 == 0 and k % 128 == 0

    # Type and shape checks
    assert m == m_ and n == n_ and k == k_
    assert m > 0 and k > 0
    assert lhs_scales.shape == ((m + 127) // 128, (k + 127) // 128)
    ...
    num_sms = get_num_sms()
    block_m, block_n, num_stages, num_tma_multicast, smem_size = get_best_configs(m, n, k, 1, num_sms, swapAB=True)
    args = (lhs, ld_lhs * 1, lhs_scales, rhs, ld_rhs * 1, rhs_scales, out, ld_out * 2, n, torch.cuda.current_stream(), num_sms, smem_size)
    runtime = jit_tuner.compile_and_tune(
        name='gemm_fp8_fp8_bf16_nt_swapAB',
        keys={'M': m, 'K': k, 'BLOCK_M': block_m, 'BLOCK_N': block_n,
              'NUM_STAGES': num_stages, 'NUM_TMA_MULTICAST': num_tma_multicast},
        space=(),
        includes=includes,
        arg_defs=(('lhs', torch.float8_e4m3fn), ('ld_lhs_in_bytes', int), ('lhs_scales', torch.float),
                  ('rhs', torch.float8_e4m3fn), ('ld_rhs_in_bytes', int), ('rhs_scales', torch.float),
                  ('out', torch.bfloat16), ('ld_out_in_bytes', int), ('n', int),
                  ('stream', torch.cuda.Stream), ('num_sms', int), ('smem_size', int)),
